    ...
    (keys, callback) => {
      ...
      if (keys === null) {
        ...
        Object.keys(keys).forEach((key) => {
          (result as any)[key] =
            data.sync[key] !== undefined ? data.sync[key] : (keys as any)[key];
        });
        callback(result);
      }
    }
...
export const createMockTweetElement = (
  options: {
    username?: string;
    hasVideo?: boolean;
    videoCount?: number;
...
export const simulateRuntimeMessage = (
  ...
  const listeners = (chrome.runtime.onMessage.addListener as jest.Mock).mock
    .calls;
...
export const createMockDownloadRecord = (
  overrides: Partial<MockDownloadRecord> = {}
): MockDownloadRecord => ({
  id: 123,
  tweetId: '123456789',
