...
export class Analytics {
  ...
  static async getOrCreateClientId() {
    ...
    if (!clientId) {
      ...
      clientId =
        self.crypto.randomUUID(); /* eslint-disable-line no-restricted-globals */
  ...
  static async getOrCreateSessionId() {
    ...
    let sessionData = result.sessionData as
  ...
  static async fireEvent(name: string, params: Record<string, any> = {}) {
    ...
    try {
      const response = await fetch(
        `${Analytics.debug ? GA_DEBUG_ENDPOINT : GA_ENDPOINT}?measurement_id=${
  ...
  static async firePageViewEvent(
    pageTitle: string,
    pageLocation: string,
